...
public class PayrollController {
	...
	public ModelAndView viewgeneratepay() {
		...
		return new ModelAndView("admin/generatepay", "employeeList", employeeList)
				.addObject("attendenceList", attendenceList).addObject("employeeList1", employeeList1);
	}
	...
	public ModelAndView CalculatePayroll(@ModelAttribute EmployeeVO employeeVO, @RequestParam int employeeTableId,
		 ...
		 * this.attendenceService.findByEmpId(employeeTableId);
		 * List emplBasicSalaryList =
		 * this.payrollservice.findByDesignationID(designationId);
		 * List emplAllowanceDeductionList =
		 * this.allowanceDeductionService.findByDesignationId(designationId);
		 * 
		 ...
		 * {
		 * //allowanceDeductionVO = (AllowanceDeductionVO)
		 * emplAllowanceDeductionList.get(j);
		 ...
		 * }
		 * }
		 * double emplfinalPayroll = this.payrollservice.calculatePayroll(workingDays,
		 * presentDays, basicSalary , allowanceLimit, deductionLimit );
		...
		String fileName = Basemethods.generatePaySlip(employeeVO2, attendanceVO,
				path + "//" + employeeVO2.getEmployeeId(), basicSalary, allowanceList, deductionList);
